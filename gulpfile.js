...
const animate_css_path = './node_modules/animate.css/animate.min.css';
...
function copy_static_js(done) {
    gulp.src([jquery_path, bootstrap_js_path, popper_js_path]).pipe(gulp.dest('./assets/js/'));
    done();
}

function copy_static_css(done) {
    gulp.src([bootstrap_css_path, animate_css_path]).pipe(gulp.dest('./assets/css/'));
    done();
}
